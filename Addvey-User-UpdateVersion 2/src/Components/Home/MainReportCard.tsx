...
const MainReportCard = ({item, reportData, from}) => {
  const [activeIndex, setActiveIndex] = useState(0);
console.log(item?.images)
  const renderImageItem = ({ item }: { item: any }) => (
    <View style={{ width: width }}>
      <Image source={{uri: ImageBackgroundBase + item}} style={styles.cardImage} />
    </View>
  );
...
const navigation = useNavigation()
  return (
    <TouchableOpacity style={{ marginBottom: hp(3) }} onPress={() => !from && navigation.navigate('BuySellContactUsSelectScreen', {item:item, reportData:reportData})} activeOpacity={1}>
      <View style={styles.card}>
        ...
        <View style={styles.profileSection}>
          ...
          <View style={{ flex: 1, marginLeft: wp(2) }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <Text style={styles.profileRole}> vender</Text>
              <Image
        ...
        <View style={styles.cardContent}>
          <View style={styles.row}>
            ...
            <Text style={styles.rowRightText}>{item?.createdAt?.split('T')[0]}</Text>
          </View>
        ...
        <View style={styles.footerContainer}>
          <Text style={styles.cardFooter}>AD ID : {item?.id}</Text>
        </View>
      </View>
